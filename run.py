...
def lane_detector(lane_area, point, option_val, class_ID):   
   left_lane = cv2.pointPolygonTest(np.array(lane_area[0]), point, False) #check if point in the left lane or not
...
def draw_bouding_box(frame, label, xmin, ymin, xmax, ymax, isWrong):
   ...
   if(isWrong == False):
      cv2.rectangle(frame, (xmin, ymin), (xmax, ymax), box_color_alert, thickness)
      cv2.rectangle(frame, (xmin, ymin - label_height - 10), (xmin + label_width, ymin), box_color_alert, -1)
